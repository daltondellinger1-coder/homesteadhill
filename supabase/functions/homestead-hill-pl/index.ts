...
async function assertAdmin(req: Request) {
  ...
  if (!allowlist?.length) {
    return { ok: false as const, status: 403, error: 'Not authorized for Homestead Hill P&L' }
  }
...
async function getLiveSnapshotIfConfigured() {
  const liveUrl = Deno.env.get('HOMESTEAD_HILL_PL_LIVE_JSON_URL')
  const liveToken = Deno.env.get('HOMESTEAD_HILL_PL_LIVE_JSON_TOKEN')
  ...
  if (!response.ok) {
    throw new Error(`Live Homestead Hill JSON fetch failed: ${response.status}`)
  }

  const liveData = await response.json()
  if (!liveData?.transactions?.length || !liveData?.months?.length) {
    throw new Error('Live Homestead Hill JSON response has invalid dashboard shape')
  }
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.error('Falling back to embedded Homestead Hill snapshot:', error)
  }
